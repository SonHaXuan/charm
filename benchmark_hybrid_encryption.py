...
class HybridIBEBenchmark:
    def __init__(self):
        self.group = PairingGroup('SS512')
        self.ibe = IBE_BonehFranklin(self.group)
        self.pk, self.mk = self.ibe.setup()
        self.id_str = "alice@example.com"
        self.sk_id = self.ibe.extract(self.mk, self.id_str)
    ...
    def encrypt_hybrid(self, message):
        ...
        try:
            ...
            # Step 2: Encrypt seed using IBE
            ciphertext_ibe = self.ibe.encrypt(self.pk, self.id_str, seed)
    ...
    def decrypt_hybrid(self, ciphertext_ibe, ciphertext_sym, max_retries=2):
        ...
        while retry_count <= max_retries:
            try:
                ...
                # Step 1: Decrypt seed using IBE
                decrypted_seed = self.ibe.decrypt(self.pk, self.sk_id, ciphertext_ibe)
                
                # Validate seed - if None, try again
                if decrypted_seed is None:
                    if retry_count < max_retries:
                        retry_count += 1
                        print(f"    IBE decryption returned None, retrying ({retry_count}/{max_retries})...")
                        continue
                    else:
                        raise ValueError("IBE decryption failed after all retries - returned None")
                
                if not isinstance(decrypted_seed, bytes):
                    raise ValueError(f"Invalid decrypted seed type: {type(decrypted_seed)}")
                
                if len(decrypted_seed) != 32:
                    raise ValueError(f"Invalid decrypted seed length: {len(decrypted_seed)}")
                
                # Step 2: Reconstruct symmetric key
    ...
    def print_results(self, results):
        ...
        print("\n" + "="*120)
        print("HYBRID IBE ENCRYPTION/DECRYPTION BENCHMARK RESULTS")
        print("="*120)
...
def main():
    ...
    print("Starting Hybrid IBE Encryption Benchmark...")
    ...
    import json
    with open('/Users/thanhtuan/son/charm/benchmark_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\nResults saved to: /Users/thanhtuan/son/charm/benchmark_results.json")
    
    # Generate CSV files
    try:
        from create_benchmark_csv import create_benchmark_csv
        create_benchmark_csv()
    except ImportError:
        print("CSV generation script not found. Run create_benchmark_csv.py separately.")
